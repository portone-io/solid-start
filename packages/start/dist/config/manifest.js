    ...
    return {
        ...
        async resolveId(id) {
            ...
            if (id === VIRTUAL_MODULES.middleware) {
                if (start.middleware)
                    return await this.resolve(start.middleware);
                return `\0${VIRTUAL_MODULES.middleware}`;
            }
        },
        async load(id) {
            if (id === `\0${VIRTUAL_MODULES.clientViteManifest}`) {
                let clientViteManifest;
                if (this.environment.config.command === "serve") {
                    clientViteManifest = {};
                }
                else {
                    const entry = Object.values(globalThis.START_CLIENT_BUNDLE).find(v => "isEntry" in v && v.isEntry);
                    if (!entry)
                    ...
                    if (manifestKey && viteMajor < 8) {
                        ...
                        rawManifest = manifestAsset.source;
                    }
                    else {
                        try {
                            const appRoot = start.appRoot || "./src";
                            ...
                            if (devServer?.environments?.client?.config?.build?.outDir) {
                                ...
                                outDir = globalThis.START_CLIENT_OUT_DIR;
                            }
