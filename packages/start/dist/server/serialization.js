...
export function serializeToJSStream(id, value) {
...
export function serializeToJSONStream(value) {
    return new ReadableStream({
        start(controller) {
...
export async function serializeToJSONString(value) {
    const response = new Response(serializeToJSONStream(value));
    return await response.text();
}
export async function deserializeFromJSONString(json) {
    const blob = new Response(json);
    return await deserializeJSONStream(blob);
}
export async function deserializeJSONStream(response) {
    if (!response.body) {
    ...
    if (!result.done) {
        ...
        function interpretChunk(chunk) {
            const value = fromCrossJSON(JSON.parse(chunk), {
                refs,
                disabledFeatures: DISABLED_FEATURES,
                depthLimit: MAX_SERIALIZATION_DEPTH_LIMIT,
                plugins: DEFAULT_PLUGINS,
...
export async function deserializeJSStream(id, response) {
    if (!response.body) {
        throw new Error("missing body");
    }
    const reader = new SerovalChunkReader(response.body);
